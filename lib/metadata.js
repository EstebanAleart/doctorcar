...
export function makeServicioMetadata(servicioSlug) {
  ...
  if (!s) return {};
  const titulo = `${s.nombre} en Rosario y Zona | DoctorCar`;
  const desc = `${s.descripcion_corta} Presupuesto sin cargo en 24hs. Gestión integral con tu aseguradora. DoctorCar, Rosario.`;
  return {
    title: titulo,
    description: desc,
    keywords: [s.keyword_principal, ...s.keywords_secundarias, "Rosario"].join(
      ", "
    ),
...
export function makeServicioCiudadMetadata(servicioSlug, ciudadSlug) {
  ...
  if (!s || !z) return {};
  const titulo = `${s.nombre} ${z.nombre_h1_sufijo} | DoctorCar – ${mesActual()}`;
  const desc = `${s.descripcion_corta} Atención ${z.nombre_h1_sufijo}. Presupuesto en 24hs y gestión directa con aseguradoras. Actualizado ${mesActual()}.`;
  return {
    title: titulo,
    description: desc,
    keywords: [
