  ...
  async upsertFromAuth0(auth0User) {
    ...
    const { rows } = await pool.query(
      `INSERT INTO users (id, auth0_id, email, name, role, role_id)
       ...
       ON CONFLICT(auth0_id) DO UPDATE SET email=EXCLUDED.email, name=EXCLUDED.name, role_id=EXCLUDED.role_id, updated_at=CURRENT_TIMESTAMP
       RETURNING *`,
      [id, id, auth0User.email, auth0User.name || auth0User.email, 'client']
    );
    return rows[0];
  },
