...
export function generatePDFContent(claimInput) {
  ...
  if (!claim) {
    console.error('No claim data provided');
    return undefined;
  }
  ...
  if (items.length === 0) {
    console.error('No budget items available');
    return undefined;
  }
        ...
        try {
          ...
          if (isBase64) {
            try {
              ...
              console.log("Could not load base64 photo:", e.message);
            }
            ...
            try {
              ...
              console.log("Could not load photo in client:", photoUrl);
            }
          } else {
            // Servidor: omitir imágenes ya que no tenemos base64
            console.log("Skipping photo in server-side PDF generation:", photoUrl);
          }
